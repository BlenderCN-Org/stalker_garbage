# разбирает формат *.ogf
# если запустить в Blender 2.67b, то импортируется меш без юви
# дата последних изменений - 05.05.2014

from struct import unpack
import datetime

file = open(r'c:\hero.ogf', 'rb')
s = file.read()
file.close()
        ...
        if block_id == 0x1:
            ...
            if vertex_format == ogf_vertexformat_fvf_1l or vertex_format == 1:
                ...
                for i in range(vertex_count):
                    ...
                    verts.append((mesh_data[0], mesh_data[1], mesh_data[2]))
                    # point = mesh_data[0:3]
                    normal = mesh_data[3:6]
                    u, v = mesh_data[6:8]
                    
                    # print('point:', point, file = save)
                    print('normal:', normal)
                    print('u, v:', (u, v))
            '''
            else:
    ...
    if block_id == 0x1:
        ...
        if vertex_format == ogf_vertexformat_fvf_1l or vertex_format == 1:
            ...
            for i in range(vertex_count):
                ...
                verts.append((mesh_data[0], mesh_data[2], mesh_data[1]))
                point = mesh_data[0:3]
                normal = mesh_data[3:6]
                u, v = mesh_data[6:8]
        ...
        while temp < len(indices):
            faces.append(indices[temp : temp + 3])
            temp += 3
