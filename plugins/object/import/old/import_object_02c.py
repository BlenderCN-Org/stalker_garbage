...
def parse_string(s, p):
	string = ''
	b = None	# Стартовое значение b (может быть любым кроме 0, чтобы условие входа в цикл было True)
	while b != 0:
		b = un_s(s, p)
		string += chr(b)
		p += 1
...
def parseMeshData(s):
    ...
    while len(s) > p:
        ...
        i += 1
    if useBlender:
        mesh = bpy.data.meshes.new(name)				# Создаём меш
        object = bpy.data.objects.new(name, mesh)	# Создаём объект
