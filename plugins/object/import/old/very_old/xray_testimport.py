...
def parseMeshData(s):
    ...
    for i in range(verticesCount):
        coords.append(unpack('3f', s[p : p + 12]))
    ...
    if useBlender:
        ...
        mesh.from_pydata(coords, (), faces)			# Присваиваем вершины и полигоны мешу
        ...
        faceIndex = 0
        for faceInfo in all_faces:
            if faceInfo[4] == 0:
                faceInfo = faceInfo[2:] + faceInfo[:2]

            faceUvs = []
            for i in faceInfo[1::2]:
                faceUvs = (uvs[layerIndices[i]][uvIndices[i]])
                object.data.uv_layers.active.data[i].uv[0] = faceUvs[0]
                object.data.uv_layers.active.data[i].uv[1] = 1 - faceUvs[1]
