# оригинальный плагин, но поворот корректный и юви не отражена зеркально
...
def parseMeshData(s):
    ...
    for i in range(verticesCount):
        ...
        if useBlender:
            mesh.verts.extend(coords[0], coords[2], coords[1])
    ...
    if useBlender:
        ...
        for faceInfo in faces:
            ...
            mesh.faces.extend(faceInfo[4], faceInfo[2], faceInfo[0])
            face = mesh.faces[-1]

            faceUvs = []
            for i in (faceInfo[5], faceInfo[3], faceInfo[1]):
