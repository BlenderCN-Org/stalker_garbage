    ...
    if useBlender:
        ...
        for faceInfo in faces_all:
            ...
            for i in faceInfo[1::2]:
                ...
                faceUvs.append(vert_uv[0])
                faceUvs.append(1 - vert_uv[1])
                object.data.uv_layers.active.data[i].uv[0] = vert_uv[0]
                object.data.uv_layers.active.data[i].uv[1] = 1 - vert_uv[1]
            #face.uv = faceUvs


    for i in vert_coords:
        print('v', round(i[0], 6), round(i[1], 6), round(i[2], 6))
    
    print('s off')
    
    for i in uvs[0]:
        print('vt', round(i[0], 6), round(i[1], 6))
    
    i = 0
    while i < trianglesCount:
        print('f ', faces[0][i] + 1, r'/', faceUvs[i] + 1, ' ', sep = '', end = '')
        print('f ', faces[0][i +1] + 1, r'/', faceUvs[i+1] + 1, ' ', sep = '', end = '')
        print('f ', faces[0][i+2] + 1, r'/', faceUvs[i+2] + 1, ' ', sep = '', end = '\n')
        i += 3
