...
def parseMeshData(s):
    ...
    for i in range(materialsCount):
        ...
        triangles_indices = unpack('%di' % (trianglesCount), s[p : p + 4 * trianglesCount])
        p += 4 * trianglesCount
        
        if useBlender:
            for index in triangles_indices:
                mesh.polygons[index].material_index = i
...
def parseMaterialBlock(s):
    ...
    for i in range(materialsCount):
        ...
        texture.image = image
        tex_slot = bpy.data.objects[-1].material_slots[i].material.texture_slots.add()
